...
export default function Storage({config}) {
    ...
    useEffect(()=>{
        ...
        const interval = setInterval(()=>{
            fetch(fetchAddress+encodeURI(sources[selectedSource]),config.fetchOptions).then(res=>res.json()).then(res=>setData(res))
        },1000)
        return () => clearInterval(interval)
    },[sources,sourcesLoading,selectedSource,fetchAddress,config.fetchOptions])
