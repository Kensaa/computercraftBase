...
export default function Rate({config, auth}) {
    ...
    useInterval(()=>{
        fetch(fetchAddress+encodeURI(sources[source1]),{headers: {'Authorization': 'Bearer '+info.token}}).then(res=>res.json()).then(res=>setData1(res))
        fetch(fetchAddress+encodeURI(sources[source2]),{headers: {'Authorization': 'Bearer '+info.token}}).then(res=>res.json()).then(res=>setData2(res))
    },1000)
