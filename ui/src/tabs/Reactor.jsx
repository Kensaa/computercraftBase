...
export default function Door({config, auth}) {
	...
	const { isLoading: reactorLoading, data:reactors, error: reactorError } = useFetch(address+"api/reactors", 	{headers: {'Authorization': 'Bearer '+info.token}}
	)

	const onClick = (action) => {
		fetch(address+"api/reactor", {method:'post',headers: {'Content-Type': 'application/json','Authorization': 'Bearer '+info.token},body:JSON.stringify({reactor:reactors[reactor],action})})
	}
	...
	return (
		<Container fluid className='m-0 p-0 h-100'>
			...
			{reactorLoading 
				? <p>Loading...</p> 
				: reactorError ? <p>Error: {reactorError.message}</p> 
