...
export default function handler(req: Request<z.infer<typeof querySchema>, z.infer<typeof bodySchema>>, res: Response) {
    ...
    if (actions.length === 0) return res.status(404).send('no action found for this client')
    if (!actions.includes(body.action)) return res.status(404).send('action not found')
